...
void RawImuBridge::rawCallback(const ros_arduino_msgs::RawImuConstPtr& raw_msg)
{
  ...
  else
  {
    if (use_accelerometer_ || use_gyroscope_)
    {
      ...
      //ROS_INFO("imu/data_raw Angular[%.1f,%.1f,%.1f] Liner[%.1f,%.1f,%.1f]",raw_msg->raw_angular_velocity.x,  raw_msg->raw_angular_velocity.y,  raw_msg->raw_angular_velocity.z,imu_msg->linear_acceleration.x, imu_msg->linear_acceleration.y, imu_msg->linear_acceleration.z);
