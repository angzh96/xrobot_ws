...
xrobotImuBridge::xrobotImuBridge(ros::NodeHandle nh, ros::NodeHandle pnh):
  ...
  is_calibrated_(false)
{
  ...
  raw_sub_ = nh_.subscribe("raw_imu", 5, &xrobotImuBridge::rawCallback, this);

  imu_cal_srv_ = nh_.advertiseService("imu/calibrate_imu", &xrobotImuBridge::calibrateCallback, this);
  ...
  if (use_accelerometer_ || use_gyroscope_)
  {
    ...
    xrobotImuBridge::fillRowMajor(linear_acc_covar_, linear_acc_stdev_);

    pnh_.param<double>("imu/angular_vel_stdev", angular_vel_stdev_, 0.0);
    xrobotImuBridge::fillRowMajor(angular_vel_covar_, angular_vel_stdev_);
  }
  ...
  if (use_magnetometer_)
  {
    ...
    if (use_mag_msg_)
    {
      ...
      xrobotImuBridge::fillRowMajor(magnetic_field_covar_, magnetic_field_stdev_);
    }
    else
    {
  ...
  ROS_INFO("Starting Xrobot Imu Bridge.");

}


void xrobotImuBridge::rawCallback(const ros_arduino_msgs::RawImuConstPtr& raw_msg)
{
  ...
  else
  {
    if (use_accelerometer_ || use_gyroscope_)
    {
      ...
      ROS_INFO("imu/data_raw Angular[%.1f,%.1f,%.1f] Liner[%.1f,%.1f,%.1f]", raw_msg->raw_angular_velocity.x,  raw_msg->raw_angular_velocity.y,  raw_msg->raw_angular_velocity.z,raw_msg->raw_linear_acceleration.x, raw_msg->raw_linear_acceleration.y, raw_msg->raw_linear_acceleration.z);
      ROS_INFO("imu/data_raw Liner[%.1f,%.1f,%.1f] Angular[%.1f,%.1f,%.1f]", imu_msg->angular_velocity.x,  imu_msg->angular_velocity.y,  imu_msg->angular_velocity.z,imu_msg->linear_acceleration.x, imu_msg->linear_acceleration.y, imu_msg->linear_acceleration.z);
    }
    ...
    if (use_magnetometer_)
    {
      ...
      if (use_mag_msg_)
      {
        ...
        mag_msg->magnetic_field.x = raw_msg->raw_magnetic_field.x;
        mag_msg->magnetic_field.y = raw_msg->raw_magnetic_field.y;
        mag_msg->magnetic_field.z = raw_msg->raw_magnetic_field.z;
        mag_msg->magnetic_field_covariance = magnetic_field_covar_;

        mag_pub_.publish(mag_msg);
        ROS_INFO("imu/mag [%f,%f,%f]", mag_msg->magnetic_field.x, mag_msg->magnetic_field.y, mag_msg->magnetic_field.z);
      }
      else
      {
        ...
        mag_pub_.publish(mag_msg);
        ROS_INFO("imu/mag [%f,%f,%f]", mag_msg->vector.x, mag_msg->vector.y, mag_msg->vector.z);
      }
...
bool xrobotImuBridge::calibrateCallback(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response)
{
...
void xrobotImuBridge::fillRowMajor(boost::array<double, 9> & covar, double stdev)
{
