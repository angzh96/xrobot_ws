...
def main():
    ...
    cmd_vel_publisher = rospy.Publisher("turtle1/cmd_vel", Twist)
    ...
    while not rospy.is_shutdown():
        if state == STATE_RAMP_UP:#状态提升
        ...
        elif state == STATE_RAMP_LEVEL:#状态斜坡水平
            if count > count_max: # 0.5s
                ...
                count = count + 1
        elif state == STATE_RAMP_DOWN:#状态斜坡下降
        ...
        if publish:
            ...
            # How to fake it when it's not publishing a cmd velocity? Up to the velocity controller there当它不发布一个命令速度时，如何伪造它？到那里的速度控制器
            odom.twist.twist.linear.x = cmd_vel.linear.x
